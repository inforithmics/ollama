...
type Scheduler struct {
	...
	loadFn       func(req *LlmRequest, f *ggml.GGML, dggml *llm.GGML, gpus discover.GpuInfoList, numParallel int)
	newServerFn  func(gpus discover.GpuInfoList, model string, f *ggml.GGML, adapters []string, projectors []string, draftModel string, dggml *llm.GGML, opts api.Options, numParallel int) (llm.LlamaServer, error)
...
func (s *Scheduler) load(req *LlmRequest, f *ggml.GGML, dggml *llm.GGML, gpus discover.GpuInfoList, numParallel int) {
	if numParallel < 1 {
		numParallel = 1
	}
	...
	llama, err := s.newServerFn(gpus, req.model.ModelPath, f, req.model.AdapterPaths, req.model.ProjectorPaths, req.model.DraftModelPath, dggml, req.opts, numParallel)
	if err != nil {
...
func pickBestFullFitByLibrary(req *LlmRequest, f *ggml.GGML, dggml *llm.GGML, gpus discover.GpuInfoList, numParallel *int) discover.GpuInfoList {
	...
	for _, gl := range gpus.ByLibrary() {
		...
		for _, p := range numParallelToTry {
			...
			if !envconfig.SchedSpread() {
				for _, g := range sgl {
					if ok, estimatedVRAM = llm.PredictServerFit([]discover.GpuInfo{g}, f, req.model.AdapterPaths, req.model.ProjectorPaths, dggml, req.opts); ok {
		...
		for _, p := range numParallelToTry {
			req.opts.NumCtx = req.origNumCtx * p
			if ok, estimatedVRAM = llm.PredictServerFit(sgl, f, req.model.AdapterPaths, req.model.ProjectorPaths, dggml, req.opts); ok {
...
func pickBestPartialFitByLibrary(req *LlmRequest, f *ggml.GGML, dggml *llm.GGML, gpus discover.GpuInfoList, numParallel *int) discover.GpuInfoList {
	if *numParallel <= 0 {
		*numParallel = 1
	...
	for i, gl := range byLibrary {
		_, estimatedVRAM := llm.PredictServerFit(gl, f, req.model.AdapterPaths, req.model.ProjectorPaths, dggml, req.opts)
...
func (s *Scheduler) maybeFindCPURunnerToUnload(req *LlmRequest, f *ggml.GGML, dggml *llm.GGML, gpus discover.GpuInfoList) *runnerRef {
	slog.Debug("evaluating if CPU model load will fit in available system memory")
	estimate, draftEstimate := llm.EstimateGPULayers(gpus, f, req.model.ProjectorPaths, dggml, req.opts)
