...
func TestLoad(t *testing.T) {
	...
	s.newServerFn = func(gpus discover.GpuInfoList, model string, f *ggml.GGML, adapters []string, projectors []string, draftModel string, dggml *llm.GGML, opts api.Options, numParallel int) (llm.LlamaServer, error) {
...
func (scenario *reqBundle) newServer(gpus discover.GpuInfoList, model string, f *ggml.GGML, adapters []string, projectors []string, draftModel string, dggml *llm.GGML, opts api.Options, numParallel int) (llm.LlamaServer, error) {
...
func TestHomogeneousGPUs(t *testing.T) {
	...
	s.newServerFn = func(gpus discover.GpuInfoList, model string, f *ggml.GGML, adapters []string, projectors []string, draftModel string, dggml *llm.GGML, opts api.Options, numParallel int) (llm.LlamaServer, error) {
		...
		return a.newServer(gpus, model, f, adapters, projectors, draftModel, dggml, opts, numParallel)
	}
	slog.Info("a")
	s.pendingReqCh <- a.req
	require.Len(t, s.pendingReqCh, 1)
